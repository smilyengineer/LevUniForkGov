...
contract Lev {
    ...
    string public constant name = "Leverage";
    ...
    uint public totalSupply = 1_000_000_000e18; // 1 billion Lev
    ...
    constructor(address account, address minter_, uint mintingAllowedAfter_) public {
        require(mintingAllowedAfter_ >= block.timestamp, "Lev::constructor: minting can only begin after deployment");
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "Lev::setMinter: only the minter can change the minter address");
    ...
    function mint(address dst, uint rawAmount) external {
        require(msg.sender == minter, "Lev::mint: only the minter can mint");
        require(block.timestamp >= mintingAllowedAfter, "Lev::mint: minting not allowed yet");
        require(dst != address(0), "Lev::mint: cannot transfer to the zero address");

        // record the mint
        mintingAllowedAfter = SafeMath.add(block.timestamp, minimumTimeBetweenMints);

        // mint the amount
        uint96 amount = safe96(rawAmount, "Lev::mint: amount exceeds 96 bits");
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "Lev::mint: exceeded mint cap");
        totalSupply = safe96(SafeMath.add(totalSupply, amount), "Lev::mint: totalSupply exceeds 96 bits");

        // transfer the amount to the recipient
        balances[dst] = add96(balances[dst], amount, "Lev::mint: transfer amount overflows");
