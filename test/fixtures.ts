...
export async function governanceFixture(
  ...
  // deploy LEV, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const lev = await deployContract(wallet, Lev, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, lev.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { lev, timelock, governorAlpha }
}
