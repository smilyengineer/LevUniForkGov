...
describe('scenario:FeeTo', () => {
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Lev, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Lev, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
